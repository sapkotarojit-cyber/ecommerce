@extends('frontend.frontend-layout')

@section('title', 'Order Details - ' . ($order->tracking_number ?? $order->id))

<title>Show Order - EmpireInnovation</title>


@section('content')
<div class="py-8 px-4 sm:px-6 lg:px-8">
<div class="max-w-4xl mx-auto">

    {{-- Header --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <a href="{{ route('orders.index') }}"
               class="text-sm font-medium text-indigo-600 hover:text-indigo-800">
                <i class="fa-solid fa-arrow-left mr-2"></i>Back to Orders
            </a>

            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mt-2">
                Order #{{ $order->tracking_number ?? $order->id }}
            </h1>
        </div>

        <div class="flex gap-3 flex-wrap">

            {{-- Invoice --}}
            <a href="{{ route('orders.invoice', $order->id) }}"
               target="_blank"
               class="px-4 py-2 bg-white border border-gray-300 rounded-xl text-sm font-semibold text-gray-700 hover:bg-gray-50">
                <i class="fa-solid fa-print mr-2"></i>Print Invoice
            </a>

            {{-- Cancel --}}
            @if($order->order_status === 'pending')
                <a href="{{ route('orders.cancel.create', $order->id) }}"
                   class="px-4 py-2 bg-red-50 border border-red-200 rounded-xl text-sm font-semibold text-red-600 hover:bg-red-100">
                    <i class="fa-solid fa-ban mr-2"></i>Cancel Order
                </a>
            @endif

            {{-- Return --}}
            @if(in_array($order->order_status, ['completed', 'cancelled']) &&
                $order->returnRequests->isEmpty())

                <a href="{{ route('orders.returns.create', $order->id) }}"
                   class="px-4 py-2 bg-amber-50 border border-amber-200 rounded-xl text-sm font-semibold text-amber-700 hover:bg-amber-100">
                    <i class="fa-solid fa-rotate-left mr-2"></i>Request Return
                </a>

            @endif
        </div>
    </div>

    {{-- Messages --}}
    @if(session('success'))
        <div class="mb-6 p-4 bg-emerald-50 border border-emerald-200
                    text-emerald-700 rounded-xl">
            {{ session('success') }}
        </div>
    @endif

    @if(session('error'))
        <div class="mb-6 p-4 bg-red-50 border border-red-200
                    text-red-700 rounded-xl">
            {{ session('error') }}
        </div>
    @endif

    {{-- Order Information --}}
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">

            <div>
                <span class="text-xs font-semibold uppercase text-gray-400">
                    Order Date
                </span>

                <p class="text-sm font-semibold text-gray-900 mt-1">
                    {{ $order->created_at->format('M d, Y H:i') }}
                </p>
            </div>

            <div>
                <span class="text-xs font-semibold uppercase text-gray-400">
                    Payment Method
                </span>

                <p class="text-sm font-semibold text-gray-900 uppercase mt-1">
                    {{ $order->payment_method ?? 'N/A' }}
                </p>
            </div>

            <div>
                <span class="text-xs font-semibold uppercase text-gray-400">
                    Payment Status
                </span>

                <span class="inline-block px-2.5 py-0.5 rounded-full text-xs
                    font-semibold mt-1
                    {{ $order->payment_status === 'paid'
                        ? 'bg-emerald-100 text-emerald-800'
                        : 'bg-amber-100 text-amber-800' }}">
                    {{ ucfirst($order->payment_status ?? 'pending') }}
                </span>
            </div>

            <div>
                <span class="text-xs font-semibold uppercase text-gray-400">
                    Order Status
                </span>

                <span class="inline-block px-2.5 py-0.5 rounded-full text-xs
                    font-semibold mt-1
                    @if($order->order_status === 'completed')
                        bg-emerald-100 text-emerald-800
                    @elseif($order->order_status === 'cancelled')
                        bg-red-100 text-red-800
                    @elseif($order->order_status === 'confirmed')
                        bg-indigo-100 text-indigo-800
                    @else
                        bg-blue-100 text-blue-800
                    @endif">
                    {{ ucfirst($order->order_status ?? 'pending') }}
                </span>
            </div>

        </div>
    </div>

    {{-- Shipping / Store --}}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">

        {{-- Shipping --}}
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">

            <h2 class="font-semibold text-gray-900 pb-3 mb-3 border-b">
                Shipping Address
            </h2>

            @if($order->shipping_address)

                <div class="text-sm text-gray-600 space-y-1">

                    <p class="font-semibold text-gray-900">
                        {{ $order->shipping_address->name
                            ?? $order->shipping_address->title
                            ?? 'N/A' }}
                    </p>

                    <p>
                        {{ $order->shipping_address->address
                            ?? $order->shipping_address->full_address
                            ?? '' }}
                    </p>

                    <p>
                        {{ $order->shipping_address->region ?? '' }}
                    </p>

                    <p>
                        Phone:
                        {{ $order->shipping_address->phone
                            ?? $order->shipping_address->contact_no
                            ?? '' }}
                    </p>

                </div>

            @else

                <p class="text-sm text-gray-500">
                    No shipping address attached.
                </p>

            @endif

        </div>

        {{-- Store --}}
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:col-span-2">

            <h2 class="font-semibold text-gray-900 pb-3 mb-3 border-b">
                Store Information
            </h2>

            <div class="text-sm text-gray-600 space-y-1">

                <p class="font-semibold text-gray-900">
                    Store Name:
                    {{ $order->dokan->company_name
                        ?? $order->dokan->name
                        ?? 'Default Marketplace Store' }}
                </p>

                @if($order->dokan?->email)
                    <p>Email: {{ $order->dokan->email }}</p>
                @endif

                @if($order->dokan?->phone)
                    <p>Phone: {{ $order->dokan->phone }}</p>
                @endif

            </div>
        </div>

    </div>

    {{-- Items --}}
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100
                overflow-hidden mb-6">

        <div class="p-6 border-b">
            <h2 class="text-lg font-semibold text-gray-900">
                Items in this Order
            </h2>
        </div>

        <div class="overflow-x-auto">

            <table class="w-full text-left">

                <thead>
                    <tr class="bg-gray-50 border-b text-xs font-semibold
                               text-gray-500 uppercase">

                        <th class="py-3 px-6">Product</th>
                        <th class="py-3 px-6">Variant</th>
                        <th class="py-3 px-6 text-center">Qty</th>
                        <th class="py-3 px-6 text-right">Price</th>
                        <th class="py-3 px-6 text-right">Total</th>

                    </tr>
                </thead>

                <tbody class="divide-y text-sm">

                @foreach($order->order_items as $item)

                    @php
                        $images = $item->varient->images ?? [];
                        $images = is_string($images)
                            ? json_decode($images, true)
                            : (array) $images;

                        $image = $images[0] ?? null;
                        $unitPrice = $item->amount / max($item->qty, 1);
                    @endphp

                    <tr>

                        <td class="py-4 px-6">

                            <div class="flex items-center gap-3">

                                @if($image)

                                    <img src="{{ asset('storage/'.$image) }}"
                                         class="w-12 h-12 rounded-lg object-cover border">

                                @else

                                    <div class="w-12 h-12 rounded-lg bg-gray-100
                                                flex items-center justify-center
                                                text-gray-400">
                                        <i class="fa-solid fa-box"></i>
                                    </div>

                                @endif

                                <span class="font-medium text-gray-900">
                                    {{ $item->product->title ?? 'Product Unavailable' }}
                                </span>

                            </div>

                        </td>

                        <td class="py-4 px-6 text-gray-600">
                            {{ $item->varient->title ?? 'Default' }}
                        </td>

                        <td class="py-4 px-6 text-center">
                            {{ $item->qty }}
                        </td>

                        <td class="py-4 px-6 text-right">
                            Rs. {{ number_format($unitPrice, 2) }}
                        </td>

                        <td class="py-4 px-6 text-right font-semibold">
                            Rs. {{ number_format($item->amount, 2) }}
                        </td>

                    </tr>

                @endforeach

                </tbody>
            </table>

        </div>

        <div class="p-6 bg-gray-50 border-t flex justify-between items-center">

            <span class="font-semibold text-gray-900">
                Grand Total
            </span>

            <span class="text-xl font-bold text-indigo-600">
                Rs. {{ number_format($order->total_amount, 2) }}
            </span>

        </div>

    </div>

</div>
</div>
@endsection