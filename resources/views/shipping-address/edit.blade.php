<!-- resources/views/frontend/shipping-address/edit.blade.php -->
@extends('frontend.frontend-layout')

@section('title', 'Edit Shipping Address - Empireinnovation')
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8">
            <div class="flex items-center space-x-3 mb-6">
                ...
                <h1 class="text-2xl font-bold text-[#1a2a6c]">Edit Shipping Address</h1>
            </div>
            ...
            <form action="{{ route('shipping-address.update', $address) }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $address->title) }}" 
                ...
                <div>
                    ...
                    <input type="text" id="contact_no" name="contact_no" value="{{ old('contact_no', $address->contact_no) }}" 
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#c9a84c] focus:border-transparent transition-all"
                ...
                <div>
                    ...
                    <textarea id="full_address" name="full_address" rows="3" 
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#c9a84c] focus:border-transparent transition-all"
                              placeholder="Street, City, State, Postal Code" required>{{ old('full_address', $address->full_address) }}</textarea>
                    @error('full_address')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
